"""g2: Dueling + Double DQN, uniform replay, 1-step TD. Q nets on CPU."""
...
from utilities.nn.dueling import TrackingDQN, select_action_epsilon_greedy
...
def train_g2(
    *,
    ...
    lambda_baseline_iou_floor: float | None = None,
    lambda_baseline_iou_below: float = 0.42,
    log_every: int = 100,
    ...
    target = TrackingDQN(state_dim=state_dim, hidden=hidden_dim, dropout=dropout).to(dev)
    ...
    x_extra = f"  ssf_penalty={ssf_reward_penalty}"
    if epsilon_tail is not None:
        x_extra += f"  epsilon_tail={epsilon_tail} steps={epsilon_tail_steps}"
    if lambda_baseline_iou_floor is not None:
        x_extra += f"  lambda_iou_floor={lambda_baseline_iou_floor} below={lambda_baseline_iou_below}"
    print_run_header(
        "g2_double_dueling",
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g2-dqn")
    ...
    while step < total_steps:
        ...
        if done:
            ...
            if (
                lambda_baseline_iou_floor is not None
                and not np.isnan(t_ep)
                and float(t_ep) < float(lambda_baseline_iou_below)
            ):
                lam = max(lam, float(lambda_baseline_iou_floor))
                lam = float(np.clip(lam, lambda_min, lambda_max))
                env.lambda_cost = lam
            ...
            if ref_iou_early_stop is not None and ref_iou_early_stop.on_episode_end(step, t_ep):
                break
            obs, _ = env.reset()
            ep_return = 0.0
            ep_len = 0
    ...
    torch.save(
        {
            ...
            "lambda_baseline_iou_floor": float(lambda_baseline_iou_floor)
            if lambda_baseline_iou_floor is not None
            else None,
            "lambda_baseline_iou_below": float(lambda_baseline_iou_below),
        },
        save_path,
    )
    print_training_done("g2_double_dueling", save_path=save_path, final_lambda=float(env.lambda_cost))
