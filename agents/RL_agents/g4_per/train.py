"""g4: PER + importance sampling + Double DQN."""
...
from utilities.per_buffer import PrioritizedReplayBuffer
...
def train_g4(
    *,
    ...
    buf = PrioritizedReplayBuffer(
        replay_capacity, state_dim=state_dim, rng=rng, alpha=float(per_alpha)
    )
    print_run_header(
        "g4_per",
        ...
        extra_lines=f"  per_alpha={per_alpha}  per_beta_start={per_beta_start}",
    )
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g4-per")
    ...
    while step < total_steps:
        ...
        if step >= ls and step % train_freq == 0 and len(buf) >= batch_size:
            ...
            for _ in range(gu):
                ...
                loss_vec = nn.functional.smooth_l1_loss(
                    q_sa, target_q, beta=huber_beta, reduction="none"
                )
                ...
                td_abs_since_log.append(float(np.mean(td_err)))
    ...
    print_training_done("g4_per", save_path=save_path, final_lambda=float(env.lambda_cost))
