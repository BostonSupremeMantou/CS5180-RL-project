"""g6: C51 distributional DQN + projected Bellman loss."""
...
def train_g6(
    *,
    ...
    q = C51TrackingDQN(state_dim=state_dim, hidden=hidden_dim, dropout=dropout).to(dev)
    target = C51TrackingDQN(state_dim=state_dim, hidden=hidden_dim, dropout=dropout).to(dev)
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g6-c51")
    ...
    while step < total_steps:
        ...
        a = select_action_epsilon_greedy_any(q, obs_for_policy, eps, 3, dev, rng)
        ...
        if step >= ls and step % train_freq == 0 and len(buf) >= batch_size:
            for _ in range(gu):
                ...
                logits_sa = logits.gather(
                    1, a_.view(-1, 1, 1).expand(-1, 1, q.n_atoms)
                ).squeeze(1)
                with torch.no_grad():
                    ...
                    m = project_c51(next_probs, r_, d_, q.support, gamma)
                loss = -(m * F.log_softmax(logits_sa, dim=-1)).sum(dim=-1).mean()
                with torch.no_grad():
    ...
    print_training_done("g6_c51", save_path=save_path, final_lambda=float(env.lambda_cost))
