"""g5: Soft Q backup (target logsumexp) + greedy action from online net."""
...
def train_g5(
    *,
    ...
    print_run_header(
        ...
        extra_lines=f"  softq_tau={softq_tau}",
    )
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g5-softq")
        ...
        if step >= ls and step % train_freq == 0 and len(buf) >= batch_size:
            for _ in range(gu):
                ...
                with torch.no_grad():
                    ...
                    target_q = r_ + gamma * (1.0 - d_) * v_soft
                loss = F.smooth_l1_loss(q_sa, target_q, beta=huber_beta)
                opt.zero_grad()
                loss.backward()
    ...
    torch.save(
        {
            ...
            "ablation": ablation,
            "softq_tau": float(softq_tau),
        },
        save_path,
    )
    print_training_done("g5_softq", save_path=save_path, final_lambda=float(env.lambda_cost))
