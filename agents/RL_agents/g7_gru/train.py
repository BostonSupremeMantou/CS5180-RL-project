"""g7: GRU over stacked frames + Dueling head (requires stack_k >= 2)."""
...
def train_g7(
    *,
    ...
    if int(stack_k) < 2:
        raise ValueError("g7_gru requires stack_k >= 2")
    ...
    gh = int(gru_hidden_dim) if gru_hidden_dim is not None else int(hidden_dim)
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g7-gru")
    ...
    print_training_done("g7_gru", save_path=save_path, final_lambda=float(env.lambda_cost))
