"""g3: Dueling + Double DQN + n-step replay bridge (same idea as old stage2)."""
...
def train_g3(
    *,
    ...
    ns = int(n_step)
    nstep_bridge: NStepReplayBridge | None
    if ns > 1:
        nstep_bridge = NStepReplayBridge(ns, float(gamma), buf)
        gamma_bootstrap = float(gamma) ** float(max(1, ns))
    else:
        nstep_bridge = None
        gamma_bootstrap = float(gamma)
    ...
    print_run_header(
        ...
        extra_lines=f"  n_step={ns}  gamma_bootstrap={gamma_bootstrap:.6f}",
    )
    ...
    ep_actions: list[int] = []
    step = 0
    pbar = tqdm(total=total_steps, desc="g3-nstep")
        ...
        if step >= ls and step % train_freq == 0 and len(buf) >= batch_size:
            for _ in range(gu):
                ...
                with torch.no_grad():
                    ...
                    target_q = r_ + gamma_bootstrap * (1.0 - d_) * q_next
    ...
    print_training_done("g3_nstep", save_path=save_path, final_lambda=float(env.lambda_cost))
