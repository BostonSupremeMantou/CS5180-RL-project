...
class C51TrackingDQN(nn.Module):
    """Dueling C51：输出 logits (batch, n_actions, n_atoms)。"""
...
def project_c51(
    ...
    """
    将 bootstrap 分布投影到固定支撑（Bellemare et al.）。
    next_probs: (B, n_atoms)；rewards / dones: (B,) ；support: (n_atoms,)。
    """
    vmin = support[0]
    vmax = support[-1]
...
def c51_loss(
    ...
    """交叉熵：target_m 为概率目标 (B, n_atoms)，logits_sa (B, n_atoms)。"""
    logp = F.log_softmax(logits_sa, dim=-1)
    return -(target_m * logp).sum(dim=-1).mean()
